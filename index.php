...
<?php
	...
	if(@$_POST[user] != "" && @$_POST[password] != "") {
		...
		$execValUser=$conection->query($sql_valida_user);
		while($resUsers=$execValUser->fetch_object()) {
			$id_grupo=$resUsers->id_grupo;
			$username=$resUsers->username;
			$password=$resUsers->password;
			$tipo_user=$resUsers->tipo_user;
		}
...
<?php
			...
			echo 'alert("Error en la contrasena");';
			echo '</script>';
		}
	}
	...
	echo '<tr><th>CONTRASE&Ntilde;A</th><th><input type="password" name="password" value="'.@$_POST[password].'" placeholder="Clave por defecto: 1234" size=30px></th></tr>';
